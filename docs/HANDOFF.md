...
| `src/migration/replacements.py` | Attach replacements to detection-only harvested records from the `flake8-qiskit-migration` map (member-wise rename), each sandbox-verified before attaching. `load_flake8_map`, `propose_replacement`, `enrich_records`, CLI `python -m src.migration.replacements`. |
...
1. **Extend replacement coverage (88/1,153 done) + curate the frontier misses.** Replacement extraction landed (`src/migration/replacements.py`): the **`flake8-qiskit-migration`** map (142 entries) → longest-prefix match → member-wise rename → **sandbox-verify imports on 2.x** → attach. Result: **0 → 88 verified replacements** (284 proposed, the rest correctly dropped as non-importing). To go further: RAG-over-migration-guides for the long tail the flake8 map misses (method renames, ecosystem moves), same verify gate. And **curate the 2 method-form frontier misses** (`QuantumCircuit.diagonal`/`squ`) into the seed (they need last-segment matching the auto-tier withholds), refill the adversarial frontier, re-measure. *(Re-running the harvest is now crash-safe + resumable via `--out`.)*
