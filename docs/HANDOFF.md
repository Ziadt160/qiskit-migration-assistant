...
- **Built + proved the whole harvester end-to-end (Stage 1→4)** (`src/migration/harvest.py` + `src/tests/test_harvest.py`; `griffe` added as the `[harvest]` extra, lazy-imported). `mine_candidates` (Griffe diff → public removed symbols + best-effort replacement from the old docstring) → `harvest_candidates` (sandbox-verify → promote as `source="sandbox-verified"`, a new tier between curated and parser in `_score`). **Live run, real Docker + Qiskit 2.x:** mined **2,509** public removed candidates from `0.46→2.0`; a 12-candidate batch verified **12/12 removed** — including `qasm`/`bind_parameters` (which it *rediscovered* from the seed — cross-validation) plus new ones (`i`, `cast`, `has_calibration_for`, …). **106 unit tests pass.** Known weak spot: replacement extraction from docstrings is poor (~0/2509 useful) — the harvester reliably grows *detection* coverage (removed-facts) but replacements still need a better source (decorator `additional_msg`/`new_alias`, `flake8-qiskit-migration` map, or RAG).

**Top next moves (prioritized — full rationale in §12):**
1. **Do a real harvest pass + improve replacement extraction.** The pipeline is built and proven; next is operational: run `python -m src.migration.harvest --old qiskit-terra==0.46.3 --new qiskit==2.0.2 --sandbox-backend docker --promote` (≈2,500 candidates × ~2 Docker runs ≈ 1–2 h; consider filtering to documented public API to cut noise, and parallelising the sandbox), then **re-run the adversarial eval to watch detection coverage climb off ~1%.** In parallel, strengthen Stage 2 replacements: parse the deprecation decorators (`deprecate_arg.new_alias`, `deprecate_func.additional_msg`) and/or pull the `flake8-qiskit-migration` import map. *(Manual adversarial-loop cycling remains the fallback.)*
