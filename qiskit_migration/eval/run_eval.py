"""Run the migration evaluation and gate on thresholds (used by CI).
    ...
    python -m qiskit_migration.eval.run_eval               # offline gate (detection + cleanliness)
    python -m qiskit_migration.eval.run_eval --seed-only    # skip corpus parse, curated seed only
    python -m qiskit_migration.eval.run_eval --min-recall 0.9
...
from qiskit_migration.eval.dataset.golden import load_golden
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.executable:
        from qiskit_migration.eval.metrics import evaluate_executable_correctness
    ...
    if args.retrieval:
        from qiskit_migration.eval.metrics import evaluate_retrieval
    ...
    if args.e2e:
        from qiskit_migration.eval.metrics import evaluate_end_to_end
    ...
    if args.equivalence:
        from qiskit_migration.eval.metrics import evaluate_behavioral_equivalence
        from qiskit_migration.migration.equivalence import default_equivalence_sandboxes
